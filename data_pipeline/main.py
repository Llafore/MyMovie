...
def search_movies(genres: list[dict[int | str]] = None):
    ...
    dao = MediaDAO()
    movies = (fetch_top_rated_movies(pages=50) + fetch_popular_movies(pages=50) + fetch_movies_by_genres(genres, pages_by_genre=5))
...
def search_series(genres: list[dict[int | str]] = None):
    ...
    dao = MediaDAO()
    series = (fetch_top_rated_series(pages=50) + fetch_popular_series(pages=50) + fetch_series_by_genres(genres, pages_by_genre=5))
